...
void ComputeCaustics(out float3 causticColor, inout GlobalData data, Varyings IN, float3 Ambient)
{
	...
	float3 worldUV = ProjectedWorldPos(data.worldPosition, data.refractionData.a, data.pixelDepth);


	worldUV.xz += data.refractionOffset * _CausticsDistortion;
	worldUV.y += data.worldNormal.y;
